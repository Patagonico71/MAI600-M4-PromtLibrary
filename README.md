...
`notebooks/prompt_evaluation.ipynb` analyses the results. It does not generate them.

Every output was produced by hand — each prompt pasted into a fresh model chat with one case substituted in — and every score is my own, entered after reading the output against its source message and the MeQSum expert reference. The notebook loads that table, computes the aggregates, and draws the charts. Automating the scoring would have removed the part of the assignment being assessed.
